...
def classification_results_by_site(
    i_fold: int,
    model: str,
    prob: np.ndarray,
    y: np.ndarray,
    ...
    if sites.__len__() == 1:
        ...
        # Append results
        if repeated is None:
            result.append([i_fold, model, bacc, auc, f1, recall, sampling, sites[0]])
        else:
            result.append(
                [i_fold, model, bacc, auc, f1, recall, sampling, sites[0], repeated]
            )
    else:
        for site in pd.unique(sites):
            ...
            # Append results
            if repeated is None:
                result.append([i_fold, model, bacc, auc, f1, recall, sampling, site])
            else:
                result.append(
                    [i_fold, model, bacc, auc, f1, recall, sampling, site, repeated]
                )
...
def compute_regression_results(
    ...
    # Append results
    if repeated is None:
        result.append([i_fold, model, MAE, R2, sampling, sites[0]])
    return result
