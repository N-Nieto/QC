...
All sites are pooled together and then they are sampled with two different sampling methods: low_Q and high_Q.
...
for col, sampling in enumerate(sampling_list):
    ...
    site_info["Pooled_Data"][sampling] = {
    ...
    for i_fold, (train_index, test_index) in enumerate(kf_out.split(X=X, y=Y)):  # noqa
        ...
        results = classification_results_by_site(
            i_fold, "Pooled data Test", pred_test, Y_test, results, sampling, site_test
        )  # noqa
...
with open(
    ...
    / (
        "experiment_info_"
        + str(n_age_bins)
        + "_bins_pooled_data_high_low_sampling_Q.json"
    ),
    "w",
) as f:
