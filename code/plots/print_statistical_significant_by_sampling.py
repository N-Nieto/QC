...
for sampling in sampling_list:
    ...
    data_10.append({"Sampling_Q": sampling,
                 "Median p-value": significant_features["P-value"].median(),            # noqa
                 "Number of Significant features": significant_features.__len__()})       # noqa
...
data_10.append({"Sampling_Q": sampling,
                ...
                "Number of Significant features": significant_features.__len__()/random_q_repetitions})      # noqa 
...
for sampling in sampling_list:
    ...
    print("For "+sampling+": median pvalue: " + str(significant_features["P-value"].median()))                      # noqa
    print("For "+sampling+": Number of statistically significant features: " + str(significant_features.__len__())) # noqa
    data_3.append({"Sampling_Q": sampling,
                 "Median p-value": significant_features["P-value"].median(),                        # noqa
                 "Number of Significant features": significant_features.__len__()})                 # noqa
# %%
# # for the random distributions
...
for i, (cat, hue) in enumerate(zip(df['Sampling_Q'], df['Age bins'])):
    ...
    x_pos = x_index - 0.4 + (hue_index + 0.5) * bar_width  # Adjust x-pos for hue   # noqa  
    # Annotate above the correct bar
    height = df['Number of Significant features'].iloc[i]
    p_value = df['Median p-value'].iloc[i]
    ax.text(x_pos, height + 10, f"p={p_value:.1e}", ha='center', va='bottom',
            fontsize=10, color='black')

# Customize the plot
plt.title("Number of Significant Features [median p-value] by Sample Size and Quality", fontsize=12)    # noqa
plt.xlabel("Sampling_Q", fontsize=10)
